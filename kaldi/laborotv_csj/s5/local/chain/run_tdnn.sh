...
if [ $stage -le 14 ]; then
  for lang_name in ${test_lang_names}; do
    lang_dir="data/${lang_name}"
    lang_suffix="${lang_name#lang}"
    graph_dir="${dir}/graph${lang_suffix}"
    utils/mkgraph.sh \
      --self-loop-scale 1.0 \
      ${lang_dir} ${dir} ${graph_dir}
    ...
    for decode_set in $test_sets; do
      steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 --nj 10 \
        ...
        $dir/decode${lang_suffix}_${decode_set}
    done
    ...
    for decode_set in $test_sets; do
      steps/online/nnet3/decode.sh --nj 10 --cmd "$decode_cmd" \
        ...
        ${graph_dir} \
        data/${decode_set}_hires \
        ${dir}_online/decode${lang_suffix}_${decode_set}
    done
  done
fi
exit 0
