...
if [ $stage -le 1 ]; then
  ...
  if [ ! -e data/${train_set}_sp/feats.scp ]; then
    ...
    # Especially in LaboroTVSpeech + CSJ training, MFCC files may already exist.
    echo "$0: data/${train_set}_sp/feats.scp already exists, so we're not going to recompute the MFCCs"
  fi
fi
...
if [ $stage -le 3 ]; then
  ...
  for datadir in ${train_set}_sp $dev_set ${test_sets}; do
    if [[ ! -f data/${datadir}_hires/feats.scp ]]; then
      ...
      if [ "${data_dir}" = "${train_set}_sp" ]; then
